...
export default function handler(
    ...
    if (!pass || pass !== ListUserPassPhase) {
        res.status(200).send('Invalid pass phase, try "/api/user/list?pass=****"')
    } else {
        const list = users.map(userRecord => ({ name: userRecord.name, encoded: Base64Encoder.encode(userRecord.name) || '<CORRUPTED>' }))
