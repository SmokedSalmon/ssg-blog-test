...
export const BasicAES128Encoder = (() => {
    ...
    return {
        ...
        decode: (encodedData = '') => {
            const invalidDecodeMsg = 'Cannot decode content, please check your whether iv & secretKey pair is matched. Or perhaps it is invalid/corrupted.'
            try {
...
export const Base64Encoder = (() => {
    return {
        encode: (raw = '') => {
            if (typeof raw !== 'string') throw 'Cannot encode non-string content'
            return Buffer.from(raw, 'utf-8').toString('base64').replace(/=+$/, '') // remove redundant '='
        },
        decode: (encodedData = '') => {
            const invalidDecodeMsg = 'Cannot decode content, please check your whether iv & secretKey pair is matched. Or perhaps it is invalid/corrupted.' 
            try {
